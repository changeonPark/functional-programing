...
!(function (G) {
  ...
  function merge_args(args1, args2, args3) {
    if (!args2.length)
      return args3 ? _to_unde(args1, args3) : _to_unde(_.clone(args1))
  ...
  each(
    ["Arguments", "String", "Number", "Date", "RegExp", "Error"],
    function (name) {
  ...
  // Pipeline
  _.go = function (v, _fs) {
    if (this != _ && this != window)
      return _.is_fn(_fs)
        ? goapply(this, v, arguments, 1)
        : goapply(this, v, _fs)
    var i = 0,
      fs = arguments,
      f,
  ...
  function mr_cat() {
    for (
      var args = { length: 0, __mr: true }, i = 0, len = arguments.length;
      i < len;
      i++
    ) {
      var arg = arguments[i]
      ...
      try {
        if (v) {
          if (v.__mr) {
            ...
            v = f.apply(self, v)
            continue
          } else if (v.then && _.is_fn(v.then))
            return go_async(self, v, fs, i - 1)
        }
        if (need_catch ? !f.__catch_pipe : f.__catch_pipe) continue
  ...
  function unpack_promise(v) {
    return is_mr(v)
      ? thenable_mr(v) &&
          _.go(Promise.all(PA == Promise.all ? v : _.toArray(v)), to_mr)
      : thenable(v) && v
  }
    ...
    return new Promise(function (rs, rj) {
      ...
      ;(function go(v) {
        do {
          ...
          if ((need_catch ? !fs[i].__catch_pipe : fs[i].__catch_pipe) && i++)
            continue
          need_catch = false
          try {
            v = is_mr(v)
  ...
  _.all2 = function (args) {
    ...
    for (var i = 1, l = arguments.length; i < l; i++) {
      tmp = _.is_mr(args)
        ? arguments[i].apply(this == _ ? null : this, args)
        : arguments[i].call(this == _ ? null : this, args)
      if (_.is_mr(tmp))
  ...
  _.spread2 = function (args) {
    ...
    for (var i = 0, fl = fns.length, al = args.length; i < fl || i < al; i++) {
      tmp = _.is_mr(args[i])
        ? (fns[i] || _.idtt).apply(this == _ ? null : this, args[i])
        : (fns[i] || _.idtt).call(this == _ ? null : this, args[i])
      if (_.is_mr(tmp))
        for (var j = 0, l = tmp.length; j < l; j++) res.push(tmp[j])
      else res.push(tmp)
    }
  ...
  _.all = _.All = function () {
    ...
    if (_.isArray(fns))
      return _.all2.apply(this, [to_mr(_.initial(arguments))].concat(fns))
  ...
  _.spread = _.Spread = function () {
    var fns = _.last(arguments)
    if (_.isArray(fns))
      return _.spread2.apply(this, [to_mr(_.initial(arguments))].concat(fns))
    fns = _.toArray(arguments)
    return function () {
  ...
  _.if = _.If = function (predi, fn) {
    ...
    function If() {
      ...
      return _.go.call(
        ...
        function (fnset) {
          return fnset ? fnset[1].apply(context, args) : void 0
        }
      )
    }
  }
  ...
  _.v = _.val = function (obj, key, keys) {
    ...
    return key == null
      ? void 0
      : (function v(obj, i, keys, li) {
          return (obj = obj[keys[i]])
            ? li == i
              ? obj
              : v(obj, i + 1, keys, li)
            : li == i
  ...
  _.keyval =
    _.obj =
    _.object =
      function f(list, vals) {
        var obj = {}
        if (_.isString(list)) obj[list] = vals
        else
          for (var i = 0, l = getLength(list); i < l; i++)
  ...
  _.obj2 = _.object2 = function f(obj, ks1, ks2) {
    return arguments.length == 2
      ? _(f, _, obj, ks1)
      : _.obj(_.wrap_arr(ks1), _.values(_.pick(obj, ks2)))
  }
  _.keys = _keys
  function _keys(obj) {
  ...
  _.nest = function f(key, val) {
    return arguments.length == 1
      ? _(f, key)
      : _.reduceRight(key.split("."), _.valkey, val)
  }
  _.invert = _invert
  ...
  _.is_empty = _.isEmpty = function (v) {
    return (
      (likearr(v) && (_.isArray(v) || _.isString(v) || _.isArguments(v))
        ? v
        : _keys(v)
      ).length === 0
    )
  ...
  _.memoize2 = (function (mid) {
    return function (fn) {
      ...
      return function (obj) {
        return _.has(
          obj._memoize || (obj._memoize = function () {}),
          memoize_id
        )
          ? obj._memoize[memoize_id]
          : (obj._memoize[memoize_id] = f(obj))
      }
    }
  })(0)
  ...
  function times2(len, fn) {
    for (var i = 1; i <= len; i++) fn(i)
  }
  _.times = function (len, iter) {
    for (var i = 0; i < len; i++) iter(i)
  }
  ...
  function setter(r, s) {
    for (var key in s) r[key] = s[key]
  }
  function dsetter(r, s) {
    for (var key in s) if (!_.has(r, key)) r[key] = s[key]
  }
  ...
  function flat(new_arr, arr, noDeep, start) {
    each(
      arr,
      function (v) {
        if (!likearr(v) || (!_.isArray(v) && !_.isArguments(v)))
          return new_arr.push(v)
        noDeep
          ? each(v, function (v) {
              new_arr.push(v)
            })
  ...
  function collf(fn, asy_fn) {
    return function f(d, i) {
      if (arguments.length == 1 && (_.is_fn(d) || typeof d == "string"))
        return _(f, ___, d)
      ...
      return (
        mp && (mp.__mr ? thenable_mr(mp) : mp.then && _.is_fn(mp.then))
          ? asy_fn
          : fn
      )(data, iter, ks, mp, i, l, k)
    }
  }

  _.each = collf(
    function (data, iter, ks, evd, i, l, k) {
    ...
    function f(data, iter, ks, mp, i, l, k) {
      return _.go.async(mp, function () {
        return ++i < l
          ? f(data, iter, ks, iter(data[(k = ks ? ks[i] : i)], k, data), i, l)
          : data
      })
    }
  )
  ...
  _.sum = function f(data) {
    ...
    if (!_.is_fn(arguments[arguments.length - 1]))
      arguments[arguments.length++] = _.idtt
  ...
  _.join = function f(arr, sep) {
    return arr == undefined || typeof arr == "string"
      ? _(f, _, arr)
      : _.toArray(arr).join(sep)
  }

  var _reduce_async = function f(data, iter, keys, mp, i) {
    return _.go(mp, function (memo) {
      var key = keys ? keys[i] : i
      return (keys || data).length == i
        ? memo
        : f(data, iter, keys, iter(memo, data[key], key, data), ++i)
    })
  }
  function reduce(body, _async) {
    return function f(d, i, m) {
      ...
      return (iter._p_async ? _async : body)(
        data,
        iter,
        keys,
        memo,
  ...
  _.reduce = reduce(
    function (data, iter, keys, memo, has_memo, i) {
      if (keys) {
        ...
        if (
          memo &&
          (memo.__mr ? thenable_mr(memo) : memo.then && _.is_fn(memo.then))
        )
    ...
    function (data, iter, keys, memo, has_memo, i) {
      return _reduce_async(
        data,
        iter,
        keys,
        has_memo ? memo : keys ? data[keys[i++]] : data[i++],
        i
      )
    }
  )
  ...
  _.reduceRight = _.reduce_right = reduce(
    function (data, iter, keys, memo, has_memo) {
      if (keys) {
        var i = keys.length - 1
        memo = has_memo ? memo : data[keys[i--]]
    ...
    function (d, iter, ks, memo, has_memo) {
      var i = (ks || d).length - 1
      return _reduce_right_async(
        d,
        iter,
        ks,
        has_memo ? memo : ks ? d[ks[i--]] : d[i--],
        i
      )
    }
  )
        ...
        return mem.concat(
          i >= len
            ? _.isArray(current)
              ...
              : current
            : _.isArray(current)
            ? f(current, new_keys || (new_keys = _.rest(keys, i).join(".")))
            : void 0
        )
      },
      []
  ...
  _.sortBy = _.sort_by = collf(function (data, iter) {
    return _.pluck(
      _.map(data, function (val, idx, list) {
  ...
  _.groupBy = _.group_by = collf(function (data, iter, ks, evd, i, l, k) {
    var res = {},
      val
    while (++i < l)
      _.has(res, (evd = iter((val = data[(k = ks ? ks[i] : i)]), k, data)))
        ? res[evd].push(val)
        : (res[evd] = [val])
    return res
  })
  ...
  _.countBy = _.count_by = collf(function (data, iter, ks, evd, i, l, k) {
    var res = {}
    while (++i < l)
      _.has(res, (evd = iter(data[(k = ks ? ks[i] : i)], k, data)))
        ? res[evd]++
        : (res[evd] = 1)
    return res
  })
  ...
  _.partition = collf(function (data, iter, ks, evd, i, l, k) {
    var filter = [],
      reject = []
    while (++i < l)
      (iter(data[(k = ks ? ks[i] : i)], k, data) ? filter : reject).push(
        data[k]
      )
  ...
  _.initial = function f(ary, n, guard) {
    ...
    return slice.call(
      ary,
      0,
      Math.max(0, ary.length - (n == null || guard ? 1 : n))
    )
  }
  ...
  _.unique = _.uniq = collf(function (data, iter, ks, evd, i, l, k) {
    ...
    while (++i < l)
      if (
        tmp.indexOf((evd = iter(data[(k = ks ? ks[i] : i)], k, data))) == -1
      ) {
        tmp.push(evd)
        res.push(data[k])
      }
  ...
  _.sortedIndex = _.sorted_i = function f(d, o, i) {
    ...
    if (arguments.length > 3) {
      ...
      var iter = _.apply(
        null,
        _.last(arguments, 1).concat(_.initial(arguments, 3))
      )
    } else {
      var data = d,
        obj = o,
    ...
    while (low < high) {
      ...
      if (iter(data[mid]) < value) low = mid + 1
      else high = mid
    }
    return low
  }
  ...
  _.find_k =
    _.find_key =
    _.findKey =
      collf(function (data, iter, ks, X, i, l, k) {
  ...
  function s(convert, var_names /*, source...*/) {
    function __PTFS__() {
      ...
      return _.go(
        mr(source, arguments, self),
        insert_datas1,
        insert_datas2,
        _.idtt
      )
    }
    var source = remove_comment(
      _.map(_.rest(arguments, 2), function (str_or_func) {
    ...
    var self = { matcher: {} }
    self.matcher[s_matcher_reg1] = s_matcher(
      3,
      s_matcher_reg1,
      source,
      var_names
    ...
    self.matcher[s_matcher_reg2] = s_matcher(
      2,
      s_matcher_reg2,
      source,
      var_names
    ...
    source = convert(
      source.replace(s_matcher_reg2, "{{}}").replace(/__PJT__/g, "{{{}}}")
    )
    return __PTFS__
  }
  ...
  function s_exec(re, wrap, source, args, self) {
    ...
    return _.go(
      mr(
        source.split(re),
        has_p && _.isArray(vs) ? _.map(vs, _.async(_.idtt)) : vs
      ),
      function (s, vs) {
        var i = 0
        return mr(
  ...
  function convert_to_html(source) {
    var tag_stack = [],
      ...
      is_paragraph = 0
    ary[ary.length - 1] = ary[ary.length - 1].replace(
      REG4[btab] || (REG4[btab] = new RegExp(TAB + "{" + btab + "}$")),
      ""
    )
  ...
  function start_tag(str, tag_stack, attrs, name, cls) {
    ...
    ;(cls = _.map(
      ...
      attrs ==
        (attrs = attrs.replace(
          /class\s*=\s*((\").*?\"|(\{.*?\}|\S)+)/,
          function (match, tmp, q) {
            return (
              " class=" + '"' + cls + " " + (q ? tmp.slice(1, -1) : tmp) + '"'
            )
          }
        )) &&
  ...
  _.remove = _remove
  function _remove(arr, remove) {
    _.removeByIndex(arr, arr.indexOf(remove))
    return arr
  }
  function _pop(arr) {
    arr.pop()
      ...
      _.reduce(
        ...
        function (mem, key) {
          ...
          return !key.match(/^\((.+)\)/)
            ? !key.match(/\[(.*)\]/)
              ? mem[key]
              : (function (mem, numbers) {
                  if (
                    numbers.length > 2 ||
                    numbers.length < 1 ||
                    _.filter(numbers, function (v) {
                      return isNaN(v)
                    }).length
                  )
                  ...
                  var s = numbers[0],
                    e = numbers[1]
                  return !e
                    ? mem[s < 0 ? mem.length + s : s]
                    : slice.call(
                        mem,
                        s < 0 ? mem.length + s : s,
                        e < 0 ? mem.length + e : e + 1
                      )
                })(
                  mem,
                  _.map(RegExp.$1.replace(/\s/g, "").split("~"), _.parseInt)
                )
            : _.find(mem, _.lambda(RegExp.$1))
        },
        start
      )
    )
  ...
  _.extend(_, {
    set: function f(start, sel, val) {
      ...
      var _arr = sel.split(/\s*->\s*/),
        last = _arr.length - 1
      _set(
        _arr.length == 1 ? start : _.sel(start, _arr.slice(0, last).join("->")),
        _arr[last],
        val
      )
      return start
    ...
    remove2: function (start, sel, remove) {
      if (remove) _remove(_.sel(start, sel), remove)
        ...
        var remover =
          _arr.length == 1
            ? start
            : _.sel(start, _arr.slice(0, last).join("->"))
    ...
    extend2: function (start, sel /*, objs*/) {
      _.extend.apply(
        null,
        [_.sel(start, sel)].concat(
          _.toArray(arguments).slice(2, arguments.length)
        )
      )
      return start
    },
    ...
    pop: function (start, sel) {
      _pop(_.sel(start, sel))
      return start
    },
    shift: function (start, sel) {
      _shift(_.sel(start, sel))
      return start
    },
    push: function (start, sel, item) {
      _push(_.sel(start, sel), item)
      return start
    },
    unshift: function (start, sel, item) {
      _unshift(_.sel(start, sel), item)
      return start
    },
  })

  _.immutable = _.im = _.extend(
    function (start, sel) {
      ...
      return {
        ...
        selected: _.reduce(
          ...
          function (clone, key) {
            return !key.match(/^\((.+)\)/)
              ? /*start*/ !key.match(/\[(.*)\]/)
                ? (clone[key] = _.clone(clone[key]))
                : (function (clone, numbers) {
                    if (
                      numbers.length > 2 ||
                      numbers.length < 1 ||
                      _.filter(numbers, _.pipe(_.identity, isNaN)).length
                    )
                      return _.Err("[] sel in [num] or [num ~ num]")
                    var s = numbers[0],
                      e = numbers[1]
                    return !e
                      ...
                      : (function (clone, oris) {
                          ...
                          slice.call(
                            clone,
                            s < 0 ? clone.length + s : s,
                            e < 0 ? clone.length + e : e + 1
                          )
                        )
                  })(
                    clone,
                    map(RegExp.$1.replace(/\s/g, "").split("~"), parseInt)
                  ) /*end*/
              : (function (clone, ori) {
    ...
    },
    {
      set: function (start, sel, val) {
        var _arr = sel.split(/\s*->\s*/),
          ...
          im = _.im(
            start,
            _arr.slice(0, _arr.length == 1 ? void 0 : last).join("->")
          )
      ...
      remove2: function (start, sel, remove) {
        ...
        if (remove) _remove(im.selected, remove)
          ...
          _remove(
            _arr.length == 1
              ? im.start
              : _.sel(im.start, _arr.slice(0, last).join("->")),
            im.selected
          )
        }
        return im.start
      ...
      extend: function (start /*, objs*/) {
        return _.extend.apply(
          null,
          [_.is_array(start) ? [] : {}, start].concat(
            _.toArray(arguments).slice(1, arguments.length)
          )
        )
      },
      ...
      extend2: function (start, sel /*, objs*/) {
        ...
        _.extend.apply(
          null,
          [im.selected].concat(_.toArray(arguments).slice(2, arguments.length))
        )
        return im.start
      },
      defaults2: function (start, sel /*, objs*/) {
  ...
  _.box = function (key, val) {
    ...
    else if (!is_string && arguments.length == 1)
      for (k in key) _box_data[k] = key[k]
    var _box = function () {
      return _box_data
    }
    return _.extend(_box, {
      ...
      extend2: function (el) {
        return _.extend2.apply(
          null,
          [_box_data, make_sel(el)].concat(
            _.toArray(arguments).slice(1, arguments.length)
          )
        )
      },
      defaults2: function (el) {
        return _.defaults2.apply(
          null,
          [_box_data, make_sel(el)].concat(
            _.toArray(arguments).slice(1, arguments.length)
          )
      ...
      im: {
        ...
        extend: function () {
          return _.im.extend.apply(
            null,
            [_box_data].concat(_.toArray(arguments))
          )
        },
        defaults: function () {
          return _.im.defaults.apply(
            null,
            [_box_data].concat(_.toArray(arguments))
          )
        },
        extend2: function (el) {
          return _.im.extend2.apply(
            null,
            [_box_data, make_sel(el)].concat(
              _.toArray(arguments).slice(1, arguments.length)
            )
      ...
      return _.isString(el)
        ...
        : (function (el) {
            try {
              ...
              return (
                arguments.callee(el.parentElement.closest("[_sel]")) +
                "->" +
                selector.replace("./", "")
              )
            } catch (e) {}
  ...
  !(function (_, notices) {
    ...
    function off(name1, n2_or_n2s) {
      ...
      else if (_notice && arguments.length == 2)
        each(
          _.isString(n2_or_n2s) ? [n2_or_n2s] : n2_or_n2s,
          _(_unset, _notice)
        )
    }
    ...
    function emit_loop(emit_args, _notice, key) {
      _set(
        _notice,
        key,
        _.reject(_notice[key], function (func) {
  ...
  L.loop = function f(lazy, iter, lm) {
    ...
    return Lloop(
      Ladd(lazy, null, null, null, false),
      iter,
      lm,
      arguments.length > 2
    )
  ...
  function Ltake1(lazy, iter) {
    return L.take(
      Ladd(lazy && lazy._p_lz && lazy.is_strict ? lazy.data : lazy, iter),
      1
    )
  }
  ...
  L.strict = function f(list, limit) {
    return arguments.length == 1
      ...
      : Ladd(
          ...
          null,
          null,
          _.isNumber(limit) ? list.length < limit : limit(list)
        )
  }
  ...
  function handle(self, deferred) {
    ...
    timeoutf(function () {
      var cb = self._state == 1 ? deferred.onFulfilled : deferred.onRejected,
        ret
      if (!cb)
        return (self._state == 1 ? resovle : reject)(
          deferred.promise,
          self._value
        )
      try {
  ...
  function resovle(self, val) {
    try {
      if (val && val instanceof Promise)
        finale(_.extend({ _state: 3, _value: val }))
      else if (val && typeof val.then == "function")
        tryp(_.bind(val.then, val), self)
  ...
  function finale(self) {
    if (self._state == 2 && self._deferreds.length == 0)
      timeoutf(function () {
        self._handled ||
          _.loge("Possible Unhandled Promise Rejection:", self._value)
      })
